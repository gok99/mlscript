...
class BlockTransformer(subst: SymbolSubst):
  ...
  def applyDefn(defn: Defn): Defn = defn match
    ...
    case td @ TraitDefn(owner, isym, sym, paramsOpt, auxParams, parentPath, methods,
      ...
      val privateFields2 = privateFields.mapConserve(_.subst)
      val publicFields2 = publicFields.mapConserve(f => f._1.subst -> f._2.subst)
      val requires2 = requires.mapConserve(r => Require(r.sym, r.mod.subst, r.path))
      if (owner2 is owner) && (isym2 is isym) && (sym2 is sym) &&
          (paramsOpt2 is paramsOpt) &&
          (auxParams2 is auxParams) &&
          (parentPath2 is parentPath) &&
          (methods2 is methods) &&
        ...
        then defn else TraitDefn(owner2, isym2, sym2, paramsOpt2, auxParams2,
          parentPath2, methods2, privateFields2, publicFields2, requires2, ctor).withImpReqs(td.impReqs)
...
class BlockTransformerShallow(subst: SymbolSubst) extends BlockTransformer(subst):
  ...
  override def applyDefn(defn: Defn): Defn = defn match
    case _: FunDefn | _: ClsLikeDefn | _: TraitDefn => defn
