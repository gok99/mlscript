  ...
  def blockImpl(stats: Ls[Statement], res: Rcd \/ Term)(k: Result => Block)(using Subst): Block =
    ...
    case (d: Declaration) :: stats =>
      ...
      case cls: ClassLikeDef =>
        ...
        val (impls, requires, others) = cls.body match
          case ObjBody(Term.Blk(stats, res)) =>
            val (impls, pre) = stats.partition:
              case impl : TraitDef => true
              case _ => false
            val (requires, others) = pre.partitionMap:
        ...
        val (mtds, publicFlds, privateFlds, ctorPre) = gatherMembers(others)
  ...
  override def applyDefn(defn: Defn): Defn = defn match
    ...
    case td @ TraitDefn(owner, isym, sym, paramsOpt, auxParams, parentPath, methods, 
      ...
      def withAssigns(vars: Ls[VarSymbol]): Block = vars match
        case Nil => ctor
        case v :: vs => 
          val ref = Ref(clsSym)(Tree.Ident(clsSym.nme), 666, N).noIArgs
